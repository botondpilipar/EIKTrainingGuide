...
\begin{tikzpicture}[node distance=2cm]
    ...
    \node (init) [process, right of=start, xshift=4cm] {\begin{tabular}{c} $db \coloneqq 0$ \\ $i \coloneqq 1$ \end{tabular}};
    \node (while) [decision, below of=init, yshift=-1cm] {$i > 7$};
    \node (afterIf) [process, left of=while, xshift=-4cm] {$i \coloneqq i + 1$};
    \node (if) [decision, below of=while, yshift=-1cm] {$X[i] < 0$};
    \node (true) [process, left of=if, xshift=-4.75cm, minimum width=2.5cm] {$db \coloneqq db + 1$};
    \node (stop) [startstop, right of=while, xshift=4cm] {Stop};

    \draw [arrow] (start) -- (init);
    \draw [arrow] (init) -- (while);
    \draw [arrow] (afterIf) -- (while);
    \draw [arrow] (while) -- node[anchor=south] {I} (stop);
    \draw [arrow] (while) -- node[anchor=east] {H} (if);
    \draw [arrow] (if) -- node[anchor=south] {N} ++(-4.5cm, 0) -| ([xshift=1.25cm]afterIf.south);
    \draw [arrow] (if) -- node[anchor=east] {I} ++(0, -2cm) -| (true);
    \draw [arrow] (true.north) -| ([xshift=-0.75 cm]afterIf.south);
