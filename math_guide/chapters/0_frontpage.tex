\documentclass[../Main.tex]{subfiles}
\begin{titlepage}
	\newgeometry{top=1 in, bottom=1 in, left=1 in, right= 1 in} 
	
	% \thispagestyle{frontpage}
	\begin{figure}[t]
        \includegraphics[width=6em]{EIK_logo.png}
    \end{figure}
	\begin{center}
		\vspace*{6\baselineskip}
	
		
		{\Huge \textbf{Tanulásmódszertan tréner útmutató\\}}
		
		\large{\textit{Matematika - 2020 ősz}}\\
		
        \vspace*{1,5\baselineskip}

        \begin{center}
        \large{\textbf{Pilipár Botond, Bagyura Gábor, Sebestyén Veronika, Takács Eszter}}\\
        \end{center}
		% \large{\textbf{Supervisor: Name}}\\
		
		\vspace{1,5\baselineskip}
		
% 		\large{Master thesis, Economics and Business
% Administration}\\
% 		\large{Major: Business Analytics}\\
		
		\vspace{1,5\baselineskip}
		\large{Eötvös Loránd Tudományegyetem Informatikai Kar}\\

	\end{center}
	
    \vspace*{15\baselineskip}
    \begin{center}
    {Az EFOP-3.6.3-VEKOP-16-2017-00002. számú projektben elvégzett szakmai feladat az Európai Unió
    támogatásával, az Európai Szociális Alap társfinanszírozásával valósult meg}
    \end{center}
	
\end{titlepage}